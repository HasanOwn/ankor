    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container max-w-4xl mx-auto px-4 pt-24 pb-12">
        <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-12">
          ...
          {vocabSets.length > 0 && (
            <motion.div variants={itemVariants} className="space-y-3">
              ...
              {vocabSets.map((set) => (
                <div 
                  key={set.id} 
                  className="flex items-center gap-3 p-4 bg-card border border-border rounded-xl hover:bg-accent/50 transition-colors"
                >
                  <button
          ...
          {totalSets === 0 && (
            <motion.div variants={itemVariants} className="text-center text-muted-foreground mt-8 p-6 bg-card border border-border rounded-xl">
